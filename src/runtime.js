...
const setU64 = (ptr, offset, value) => {
	const view = new DataView(jai_exports.memory.buffer);
	view.setBigUint64(ptr + offset, value, true);
}
...
jai_imports.jsCreateInstance = (params_ptr, returns_ptr) => {
	...
	view.setBigUint64(returns_ptr + 0, object_map_counter, true);
}
...
jai_imports.jsInstanceRequestAdapter = (params_ptr, returns_ptr) => {
	...
	switch(wasm_pause()) {
		case 0: (async () => {
			...
			view.setBigUint64(returns_ptr + 0, BigInt(object_map_counter), true);
			return +1;
		})().then(wasm_resume); break;
	}
}
...
jai_imports.jsAdapterGetLimits = (params_ptr, returns_ptr) => {
	...
	setU32(limits_ptr, 0, adapter.limits.maxTextureDimension1D);
	setU32(limits_ptr, 4, adapter.limits.maxTextureDimension2D);
	setU32(limits_ptr, 8, adapter.limits.maxTextureDimension3D);
	setU32(limits_ptr, 12, adapter.limits.maxTextureArrayLayers);
	setU32(limits_ptr, 20, adapter.limits.maxBindGroups);
	setU32(limits_ptr, 24, adapter.limits.maxBindGroupsPlusVertexBuffers);
	setU32(limits_ptr, 28, adapter.limits.maxBindingsPerBindGroup);
	setU32(limits_ptr, 32, adapter.limits.maxDynamicUniformBuffersPerPipelineLayout);
	setU32(limits_ptr, 36, adapter.limits.maxDynamicStorageBuffersPerPipelineLayout);
	setU32(limits_ptr, 40, adapter.limits.maxSampledTexturesPerShaderStage);
	setU32(limits_ptr, 44, adapter.limits.maxSamplersPerShaderStage);
	setU32(limits_ptr, 48, adapter.limits.maxStorageBuffersPerShaderStage);
	setU32(limits_ptr, 52, adapter.limits.maxStorageTexturesPerShaderStage);
	setU32(limits_ptr, 56, adapter.limits.maxUniformBuffersPerShaderStage);
	setU32(limits_ptr, 60, adapter.limits.maxUniformBufferBindingSize);
	setU32(limits_ptr, 64, adapter.limits.maxStorageBufferBindingSize);
	setU32(limits_ptr, 68, adapter.limits.minUniformBufferOffsetAlignment);
	setU32(limits_ptr, 72, adapter.limits.minStorageBufferOffsetAlignment);
	setU32(limits_ptr, 76, adapter.limits.maxVertexBuffers);
	setU32(limits_ptr, 80, adapter.limits.maxBufferSize);
	setU32(limits_ptr, 84, adapter.limits.maxVertexAttributes);
	setU32(limits_ptr, 88, adapter.limits.maxVertexBufferArrayStride);
	setU32(limits_ptr, 92, adapter.limits.maxInterStageShaderVariables);
	setU32(limits_ptr, 96, adapter.limits.maxColorAttachments);
	setU32(limits_ptr, 100, adapter.limits.maxColorAttachmentBytesPerSample);
	setU32(limits_ptr, 104, adapter.limits.maxComputeWorkgroupStorageSize);
	setU32(limits_ptr, 108, adapter.limits.maxComputeInvocationsPerWorkgroup);
	setU32(limits_ptr, 112, adapter.limits.maxComputeWorkgroupSizeX);
	setU32(limits_ptr, 116, adapter.limits.maxComputeWorkgroupSizeY);
	setU32(limits_ptr, 120, adapter.limits.maxComputeWorkgroupSizeZ);
	setU32(limits_ptr, 124, adapter.limits.maxComputeWorkgroupsPerDimension);
...
jai_imports.jsAdapterGetFeatures = (params_ptr, returns_ptr) => {
	...
	const memory = jai_exports.context_alloc(jai_context, n * 4);

	const features_ptr = getU64(params_ptr, 8);
	setU64(features_ptr, 0, n);
	setU64(features_ptr, 8, memory);
	let cursor = 0;
	for (const feature of adapter.features) {
		setU32(memory, cursor, feature);
		cursor += 4;
	}
}
